...
return probcomp(function()
	...
	terra BlockCurve:__construct(bc: &BezCurve3, nblocks: uint, depth: real, minWidth: real, maxWidth: real, maxHeightChange: real, margin: real, maxAng: real, bodyGen: {&Shape}->{&Body}) : {}
		m.init(self.blocks)
		-- bc:println()
		-- Make blocks of varying width and depth
		var avgDim = 0.5*(minWidth + maxWidth)
		...
		for i=0,nblocks do
			...
			var w = boundedUniform(minWidth, maxWidth, {initialVal=avgDim})
			var d = depth
	...
	terra BlockCurveNetwork:__construct(cnet: &CurveNetwork, groundDepth: real, groundThickness: real, depth: real,
										minWidth: real, maxWidth: real, maxHeightChange: real, margin: real, maxAng: real, bodyGen: {&Shape}->{&Body})
		-- Init ground
		...
		for i=0,cnet.curves.size do
			...
			var fromBlockCurve = BlockCurve.heapAlloc(&bc3, cnet.nblocks(i), depth, minWidth, maxWidth, maxHeightChange, margin, maxAng, bodyGen)
