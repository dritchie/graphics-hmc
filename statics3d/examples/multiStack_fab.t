...
return probcomp(function()
	...
	local depth = `mm(40.0)
	local maxHeightVarPercent = `0.01
	local maxAng = `radians(1.0)
	local margin = `mm(20.0)
	...
	terra Stack:__construct(botBody: &Body, botPoint: Vec3, topBody: &Body, topPoint: Vec3, numBlocks: uint, bodyGen: {&Shape}->{&Body}) : {}
		...
		for i=0,numBlocks do
			var w = boundedUniform(minDim, maxDim, {initialVal=0.5*(minDim+maxDim)})
			var d = depth
		...
		for i=0,numBlocks-1 do
			...
			mat = Mat4.translate(boundedUniform(-maxHeightVarPercent, maxHeightVarPercent, {initialVal=0.0}) * heightaxis)
	...
	terra Arch:__construct(bodyGen: {&Shape}->{&Body},
						   topCenter: Vec3, topw: real, toph: real,
		...
		-- Create the top bar
		var topShape = Box.heapAlloc(); topShape:makeBox(topCenter, topw, depth, toph)
	...
	return terra()
		...
		var arch1 = Arch.stackAlloc(Body.oak,
									Vec3.stackAlloc(0.0, 0.0, mm(200.0)), mm(300.0), mm(10.0),
		...
		-- 							Vec3.stackAlloc(mm(150.0), 0.0, mm(300.0)), mm(350.0), mm(10.0),
