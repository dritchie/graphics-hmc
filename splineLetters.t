...
local imgTri = DoubleAlphaGrid.methods.load(image.Format.PNG, "targets/tri_alpha_25_25.png")
...
local marbleParams = {width=size, height=size, xyPeriod=12.0, xPeriod=20.0, yPeriod=30.0, turbPower=10.0}
...
local function model()
  ...
  -- Turbulence 
  local makeTurbulence = U.TurbulenceifyBySubsampling(real, {width=size, height=size, levels=5.0})
  ...
  local tgtImgConstraint = imageConstraints.RandomPosTransformedSimConstraint(real,
    {target=imgTri, softness=tgtSoftness, scale=0.005})
...
U.renderSamplesToMovie(samples, moviename, gridSaver, false)