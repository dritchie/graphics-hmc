...
local function staticsModel()
	...
	inheritance.purevirtual(RigidConnectionT, "applyForces", {}->{})
	...
	HingeT.methods.applyForces = pfn(HingeT.methods.applyForces, {ismethod=true})
	inheritance.virtual(HingeT, "applyForces")
	...
	terra CablePinT:applyForces() : {}
		...
		if self.beam.active then
			...
			for i=0,self.cables.size do
				...
				totalForce = totalForce + gen1DForce(self.location, dir)
				if totalDOF == 1 and (not collinear(dir, totalForce)) then
					totalDOF = 2
				end
			end
			self.beam:applyForce(ForceT{totalForce, self.location, totalDOF})
		end
	...
	CablePinT.methods.applyForces = pfn(CablePinT.methods.applyForces, {ismethod=true})
	inheritance.virtual(CablePinT, "applyForces")
